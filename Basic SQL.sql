...
/*Write a query that limits the response to only
the first 15 rows and includes the occurred_at,
...
but with the largest total_amt_usd for each date listed first for each date. You will notice
each of these dates shows up as unique because of the time element. When
you learn about truncating dates in a later lesson, you will better be able to tackle this
...
This is for a division by zero. You will learn how to get a solution without an error to this query when you learn
about CASE statements in a later section.
For now, you might just add some very small value to your denominator as a work around.*/

SELECT id, account_id,
	poster_amt_usd / (poster_amt_usd + gloss_amt_usd + standard_amt_usd) AS poster_share
FROM orders
...
/*Using the accounts table find all the companies whose names do not
